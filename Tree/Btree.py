...
class BTree:
    ...
    def __init__(self, t):
        ...
        :param t: Order.
        """
        # the order t subjected 2t = max key
        self.root = Node(True)
        self.t = t
    ...
    def delete(self, x, k):
        ...
        # Ensuring that a child always has at least 't' keys
        else:
...
def main():
    ...
    for i in range(customNo):
        # random insert for test
        # sorted by the key and every node contain the key and the value
        B.insert((i, randint(i, 5 * i)))
    B.printTree(B.root)
    print()

    # Delete the key at a random position
